...
sim_diffs = function( phy, model_parameters, model_method="BM" ){
	...
	if( model_method == "BM"){
		...
		ancestral_states = ape::reconstruct( tip_states, phy, method="ML" )$ace
	...
	if( is.na( ancestral_states[1] ) ){
		stop("ERROR: Ancestral states were not estimated.")
	}
	
	states = c( tip_states, ancestral_states )
...
#' @importFrom magrittr %>%
#' @export
picx = function( x, phy, var.contrasts=FALSE, model_method="BM", model_parameters=NA, n_replicates = 50 ){
